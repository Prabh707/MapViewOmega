...
export const OperatorDashboard: React.FC<OperatorDashboardProps> = ({
  ...
  const safetyReports = activeReports.filter(r => r.category === 'Safety Issue' || r.category === 'Safety Commendation');
  ...
  const handleResolveBarrier = (report: CommunityReport) => {
    onResolveReport(report.id, 'Dispatched maintenance repair crew. Barrier resolved and operational status restored.');
    if (voiceEnabled) {
      SpeechService.speak(`Barrier maintenance dispatched for ${report.stopName}. Step-free routing restored.`);
    }
  };
    ...
    <div className="space-y-8 pb-20 animate-fadeIn">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-sm text-slate-300 mt-2 max-w-2xl">
              Real-time monitoring of vehicle accessibility capacities, electric ramp lifts, passenger barrier queues, and live crowd incident dispatch.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-lg">
          ...
          <div className="text-3xl font-black text-white">{fleet.length} <span className="text-sm font-normal text-slate-400">Vehicles</span></div>
        ...
        <div className={`border rounded-2xl p-5 shadow-lg transition-all ${
          ...
          <div className={`text-3xl font-black ${barrierReports.length > 0 ? 'text-red-400' : 'text-slate-100'}`}>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-lg">
          ...
          <div className="mt-2 text-xs text-slate-400">
            Perimeter CCTV & 24/7 Blue-Light active
          </div>
        </div>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-400">Continuous feed of ramp lift diagnostics, wheelchair capacity, and driver dispatch</p>
          </div>
              ...
              {fleet.map((v) => {
                ...
                return (
                  <tr key={v.vehicleId} className="hover:bg-slate-800/40 transition">
                    ...
                    {/* Ramp Status */}
                    <td className="py-3 px-3">
                      <span className={`px-2.5 py-1 rounded text-[11px] font-bold inline-flex items-center space-x-1 border ${
                        ...
                        <span>{v.rampStatus.includes('Operational') || v.rampStatus.includes('Verified') ? '✓' : '⚠️'}</span>
                    ...
                    <td className="py-3 px-3">
                      ...
                      <div className="text-[10px] text-slate-400 font-mono">ETA: {v.etaNextStopSec}s • {v.speedKmh} km/h</div>
                    </td>
                      ...
                      {pingMsg ? (
                        ...
                        <div className="flex items-center justify-end space-x-1.5">
                          ...
                          <button
                            ...
                            title={v.emergencySosActive ? 'Clear Emergency SOS' : 'Trigger Safe Escort Protocol'}
                    ...
                    {!isResolved ? (
                      <>
                        {report.type === 'sos_alert' ? (
                          <button
                            onClick={() => onResolveReport(report.id, 'Campus Security Patrol Unit #4 (Officer J. Miller) intercepted passenger safely. Escort completed to destination.')}
                          ...
                          <button
                            onClick={() => onResolveReport(report.id, 'Campus safety escort and lighting crew dispatched.')}
                          ...
                          <button
                            onClick={() => onResolveReport(report.id, 'Extra vehicle dispatched to handle crowd load.')}
                          ...
                          <button
                            onClick={() => onResolveReport(report.id, 'Broadcasted advisory notice to all passengers.')}
