...
export const JourneyMode: React.FC<JourneyModeProps> = ({
    ...
    const interval = setInterval(() => {
      setTimerSeconds(prev => {
        if (prev <= 1) {
          ...
          if (voiceEnabled) {
            SpeechService.speak('Warning: Safety check-in missed! Please confirm you are safe within 30 seconds before emergency contacts and campus security are alerted.');
          }
          return 0;
        }
        return prev - 1;
    ...
    const warnInterval = setInterval(() => {
      setWarningSeconds(prev => {
        if (prev <= 1) {
          ...
          if (voiceEnabled) {
            SpeechService.speak('Safety check-in expired with no response. Automated emergency escalation activated. Alerting emergency contacts and campus security desk.');
          }
          return 0;
        }
        return prev - 1;
  ...
  const handleRepeatVoice = () => {
    if (voiceEnabled) {
      SpeechService.speak(`Step ${currentStepIndex + 1} of ${steps.length}: ${currentStep.instructions}. ${currentStep.accessibilityNotes ? currentStep.accessibilityNotes.join('. ') : ''}`);
    }
  };
  ...
  const handleCompleteArrival = () => {
    ...
    if (voiceEnabled) {
      SpeechService.speak('Safe arrival confirmed! Journey completed safely. Thank you for using AccessRide.');
    }
    setTimeout(() => {
      onCompleteJourney();
    }, 2500);
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  If countdown expires without confirmation, an emergency escalation sequence automatically notifies your trusted contacts and campus dispatch.
                </p>
              </div>
            </div>
      ...
      {checkinPhase === 'warning' && (
        <div className="bg-gradient-to-br from-amber-950/90 via-slate-900 to-red-950/80 border-2 border-amber-500 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-5 animate-pulse">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-300">
                  You missed your scheduled transit check-in. Please confirm you are safe before emergency escalation.
                </p>
              </div>
            </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-950/90 border border-amber-500/40 space-y-2">
            <div className="text-xs font-bold text-slate-300 uppercase tracking-wider">Automated Escalation Pipeline:</div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-red-200">
                  User did not respond to check-in warning. Emergency contacts and campus police command have been automatically mobilized.
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-5 rounded-2xl bg-slate-950/90 border border-amber-500/60 shadow-lg space-y-3">
              ...
              <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-[11px] text-slate-300 font-mono space-y-1">
                <div className="text-[10px] text-amber-400 font-bold uppercase">Automated SMS Payload Preview:</div>
                <p className="text-slate-300 italic">
                  "🚨 URGENT: Alex Rivera missed a scheduled transit check-in on 'SafeCorridor Campus Night Shuttle'. Last GPS: 42.3601° N, -71.0942° W near Central Library. Battery: 84%. Live Tracking: https://accessride.campus/track/b048"
                </p>
              </div>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-950/90 border border-red-500/60 shadow-lg space-y-3">
              ...
              <div className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-xs space-y-1">
                ...
                <div className="text-slate-300">Responding Officer: <strong>Officer J. Miller (Badge #409)</strong></div>
                <div className="text-emerald-400 font-bold">⏱️ ETA to Intercept Point: ~2 Minutes</div>
              </div>

              <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-xs space-y-1">
                <div className="text-slate-400">Intercept Location: <strong>{currentStep.fromName}</strong></div>
                <div className="text-slate-400">Operator Incident Ticket: <strong className="text-amber-400">Injected to Dispatch Queue</strong></div>
              </div>
            </div>
