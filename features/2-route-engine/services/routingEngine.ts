...
export class RoutingEngine {
  ...
  public static calculateRoutes(
    ...
    if (
      this.canUseLine(shuttleLine, originId, destId) ||
      originId === 'stop_gate' ||
      originId === 'stop_lib' ||
      destId === 'stop_lib' ||
      destId === 'stop_hosp'
    ...
    if (
      this.canUseLine(metroLine, originId, destId) ||
      originId === 'stop_gate' ||
      originId === 'stop_lib' ||
      destId === 'stop_metro' ||
      originId === 'stop_sports' ||
      destId === 'stop_tech'
    ) {
    ...
    if (
      this.canUseLine(busLine, originId, destId) ||
      originId === 'stop_gate' ||
      originId === 'stop_lib' ||
      destId === 'stop_hosp' ||
      originId === 'stop_arts'
    ...
    if (
      this.canUseLine(greenLine, originId, destId) ||
      originId === 'stop_gate' ||
      originId === 'stop_sports' ||
      destId === 'stop_tech' ||
      originId === 'stop_lib' ||
  ...
  private static buildShuttleRoute(
    ...
    return {
      ...
      summary:
        '100% step-free low-floor shuttle through well-lit campus security corridors with minimal walking.',
  ...
  private static buildMetroRoute(
    ...
    const segments: RouteSegment[] = [
      {
        ...
        accessibilityNotes: [
          ...
          hasBrokenElevator
            ? '🚨 CRITICAL: Elevator is currently OUT OF SERVICE for hydraulic repair'
            : '✓ Elevator available',
      ...
      },
      {
        ...
        accessibilityNotes: ['⚠️ 240m walk through subterranean corridor', '✓ High pedestrian traffic'],
        stepFree: false,
        hasStairs: true,
        lightingScore: 8.7,
        elevatorInvolved: true,
    ...
    return {
      ...
      summary:
        'Express underground subway. Fastest travel time, but contains stairs, a broken elevator alert, and high crowds.',
      totalDurationMin: totalDuration,
      totalWalkDistanceMeters: totalWalk,
      transferCount: 0,
      segments,
  ...
  private static buildBusRoute(
    ...
    const segments: RouteSegment[] = [
      ...
      },
      {
        ...
        accessibilityNotes: ['✓ Paved sidewalk with 1:20 gentle grade', '✓ Well-lit street section'],
        stepFree: true,
        hasStairs: false,
        lightingScore: dest.lightingScore,
        elevatorInvolved: false,
    ...
    return {
      ...
      summary:
        'Reliable street-level bus route with low-floor ramp, audio-visual announcements, and moderate walking distance.',
      totalDurationMin: totalDuration,
      totalWalkDistanceMeters: totalWalk,
      transferCount: 0,
      segments,
  ...
  private static buildGreenRailRoute(
    ...
    return {
      ...
      summary:
        'Spacious light rail with level platform boarding and zero vibration. Ideal for comfort and sensory ease, with moderate walking.',
      totalDurationMin: totalDuration,
      totalWalkDistanceMeters: totalWalk,
      transferCount: 0,
      segments,
  ...
  private static buildSyntheticDirectRoute(
    origin: TransitStop,
    dest: TransitStop,
    preferences: UserPreferences
  ): RouteCandidate {
  ...
  private static buildSyntheticAlternativeRoute(
    origin: TransitStop,
    dest: TransitStop,
    preferences: UserPreferences
  ): RouteCandidate {
  ...
  private static generatePolylineCoordinates(
    ...
    for (let i = 1; i < segmentsCount; i++) {
      ...
      const lng =
        from.lng + (to.lng - from.lng) * fraction + Math.sin(fraction * Math.PI) * (curveOffset * 0.7);
      coords.push([lat, lng]);
    }
   ...
   */
  private static calculateScores(route: RouteCandidate, preferences: UserPreferences): RouteScoreBreakdown {
      ...
      switch (preferences.profileId) {
        case 'wheelchair':
          overallScore = Math.round(
            accessibilityScore * 0.6 + safetyScore * 0.2 + comfortScore * 0.1 + speedScore * 0.1
          );
          break;
        ...
        case 'night_safety':
          overallScore = Math.round(
            safetyScore * 0.6 + accessibilityScore * 0.2 + comfortScore * 0.1 + speedScore * 0.1
          );
          break;

        case 'quiet_sensory':
          overallScore = Math.round(
            comfortScore * 0.5 + safetyScore * 0.25 + accessibilityScore * 0.15 + speedScore * 0.1
          );
          break;

        case 'vision_hearing':
          overallScore = Math.round(
            accessibilityScore * 0.5 + safetyScore * 0.3 + comfortScore * 0.1 + speedScore * 0.1
          );
          break;
        ...
        default:
          overallScore = Math.round(
            speedScore * 0.5 + accessibilityScore * 0.2 + safetyScore * 0.2 + comfortScore * 0.1
          );
          break;
      }
    }
  ...
  private static generateExplanation(
    ...
    if (route.stepFree && !route.hasStairs) {
      whyRecommended.push(
        '100% Step-Free: Complete level access, ADA curb cuts, and low-floor electric boarding ramp.'
      );
    }

    if (route.totalWalkDistanceMeters <= 100) {
      whyRecommended.push(
        `Minimal Walking: Only ${route.totalWalkDistanceMeters}m of level walking with zero steep inclines.`
      );
    } else if (route.totalWalkDistanceMeters <= preferences.maxWalkDistanceMeters) {
      whyRecommended.push(
        `Comfortable Walk: ${route.totalWalkDistanceMeters}m walking distance fits within your ${preferences.maxWalkDistanceMeters}m preference limit.`
      );
    }

    if (route.lightingAverage >= 9.2) {
      whyRecommended.push(
        `Safe & Bright: High-intensity LED lighting (${route.lightingAverage}/10) along CCTV and Blue-Light SOS corridors.`
      );
    }

    if (route.crowdLevel === 'low') {
      whyRecommended.push(
        'Low Crowding: Calm vehicle environment with open wheelchair positions and guaranteed seating.'
      );
    }
    ...
    if (route.id === 'route_shuttle_safecorridor') {
      tradeOffs.push(
        'Takes 4 minutes longer than underground subway, but avoids 18 stairs and broken elevator hazards.'
      );
    } else if (route.id === 'route_metro_express') {
      tradeOffs.push(
        'Fastest travel time (11 mins), but includes physical barriers and heavy passenger volume.'
      );
    } else if (route.id === 'route_bus4_rapid') {
      tradeOffs.push(
        'Reliable surface transit with flip-out ramp; requires crossing street at audible signal.'
      );
    } else if (route.id === 'route_green_rail') {
      tradeOffs.push(
        'Zero-vibration level rail boarding, but involves 370m total walking distance across campus.'
      );
    }
    ...
    if (route.totalWalkDistanceMeters > preferences.maxWalkDistanceMeters) {
      barrierWarnings.push(
        `🚶 Walking distance (${route.totalWalkDistanceMeters}m) exceeds your selected limit (${preferences.maxWalkDistanceMeters}m).`
      );
    }
